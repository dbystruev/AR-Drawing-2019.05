...
class OptionsContainerViewController: UIViewController, UINavigationControllerDelegate {
    ...
    private func scenePicker() -> UIViewController {
        ...
        let availableScenes: [String] = {
            ...
            return fileEnumerator.compactMap { element in
                ...
                guard url.pathExtension == "dae" || url.pathExtension == "scn" else { return nil }
        ...
        selector.optionSelectionCallback = { [unowned self] name in
            let nameWithoutExtension = name
                .replacingOccurrences(of: ".dae", with: "")
    ...
    private func shapePicker() -> UIViewController {
        let shapes: [Shape] = [.box, .sphere, .cylinder, .cone, .torus, .pyramid]
    ...
    private func colorPicker() -> UIViewController {
        let colors: [(String, UIColor)] = [
            ...
            ("Green", .green),
            ("Blue", .blue),
            ("Violet", #colorLiteral(red: 0.5568627715, green: 0.3529411852, blue: 0.9686274529, alpha: 1)),
            ("Brown", .brown),
            ("White", .white)
        ]
